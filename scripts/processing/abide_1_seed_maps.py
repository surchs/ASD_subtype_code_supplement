...
def abide1_seed(n_cpu):
    ...
    pheno_p = root_p / 'data/pheno/ABIDE1_Pheno_QC_pass.tsv'
    # Data
    fc_p = root_p / 'data/preprocessed/time_series/abide_1/'
    fc_t = 'fmri_sub{:07}_session{}_run{}_scrubbed.nii.gz'

    # Output
    out_t = f'sub_{{}}_ses_{{}}_run{{}}_mist_{scale}_nocereb.npy'
    out_p = root_p / f'data/preprocessed/seed_maps/abide_1/MIST_{scale}'
    if not out_p.is_dir():
        out_p.mkdir()
