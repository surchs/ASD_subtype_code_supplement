...
def abide1_nuisance():
    ...
    sca_p = root_p / f'data/preprocessed/seed_maps/abide_1/MIST_{scale}'
    sca_t = f'sub_{{}}_ses_{{}}_run{{}}_mist_{scale}_nocereb.npy'
    pheno_p = root_p / 'data/pheno/ABIDE1_Pheno_PSM_matched_minimum_10.tsv'
    # Output
    out_p = root_p / 'data/processed/residuals/abide_1'
    resid_p = out_p / f'abide_1_mist_{scale}_residuals_after_{"_and_".join(regressors.split("+"))}_nocereb.npy'
    if not out_p.is_dir():
        out_p.mkdir()
