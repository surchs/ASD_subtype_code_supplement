...
def hnu1_seed(n_cpu):
    ...
    # Data
    fc_p = root_p / 'data/preprocessed/time_series/hnu_1/'
    fc_t = 'fmri_{:07}_session{}_rest_scrubbed.nii.gz'

    # Output
    out_t = f'sub_{{}}_run_{{}}_mist_{scale}_nocereb.npy'
    out_p = root_p / f'data/preprocessed/seed_maps/hnu_1/MIST_{scale}'
    if not out_p.is_dir():
        out_p.mkdir()
    ...
    path_list = [(fc_p / fc_t.format(row['SUBID'], row['SESSIONID']), out_p / out_t.format(row['SUBID'], row['SESSIONID']))
                 for rid, row in pheno.iterrows()]
